...
// NewEntry creates a new c2-session entry with an empty ClaudeUUID.
...
func NewEntry(store core.ArchiveStore, cwd, name string) (core.C2Entry, error) {
	if !filepath.IsAbs(cwd) {
		return core.C2Entry{}, fmt.Errorf("%w: cwd must be absolute, got %q", ErrValidation, cwd)
	}
	info, err := os.Stat(cwd)
	if err != nil {
		return core.C2Entry{}, fmt.Errorf("%w: cwd: %v", ErrValidation, err)
	}
	if !info.IsDir() {
		return core.C2Entry{}, fmt.Errorf("%w: cwd is not a directory: %s", ErrValidation, cwd)
	}
	if name == "" {
	...
	if name == "" {
		return core.C2Entry{}, fmt.Errorf("%w: name is empty", ErrValidation)
	}
	if utf8.RuneCountInString(name) > MaxNameLen {
		return core.C2Entry{}, fmt.Errorf("%w: name exceeds %d chars", ErrValidation, MaxNameLen)
	}
